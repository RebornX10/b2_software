...
shape_to_label = {
    "rock": np.array([1.0, 0.0, 0.0]),
    "paper": np.array([0.0, 1.0, 0.0]),
    "scissor": np.array([0.0, 0.0, 1.0]),
}
...
for rounds in range(NUM_ROUNDS):
    ...
    for i in range(90):
        ...
        # Prediction
            ...
            pred = arr_to_shape[
                np.argmax(loaded_model.predict(prepImg(frame[50:350, 100:400])))
            ]
        ...
        frame = cv2.putText(
            frame,
            "Player : {}      Bot : {}".format(playerScore, botScore),
            (120, 400),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (250, 250, 0),
